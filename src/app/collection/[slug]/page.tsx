...
export async function generateMetadata({ params }: CollectionPageProps): Promise<Metadata> {
  ...
  if (!collection) {
    return {
      title: 'Collection Not Found | Marbella Live',
  ...
  const description = isPersonal
    ? `${propertyCount} hand-picked ${propertyCount === 1 ? 'property' : 'properties'} selected for ${collection.recipient_name} by Marbella Live.`
  ...
  return {
    ...
    openGraph: {
      title: isPersonal
        ...
        : collection.title,
      description: `${propertyCount} curated ${propertyCount === 1 ? 'property' : 'properties'} by Marbella Live`,
      url: collectionUrl,
