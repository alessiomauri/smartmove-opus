/**
 * Credential-scrub tests — PARITY SUITE.
 *
 * The same assertions run against BOTH implementations:
 *   - workers/resales-proxy/src/scrub.ts  (Cloudflare worker, idle fallback)
 *   - relay/resales-relay/scrub.mjs       (VPS relay, primary path)
 * A behavioural drift between them fails this suite.
 *
 * Run: npx tsx workers/resales-proxy/test-scrub.mjs
 */
const implementations = [
  ['worker scrub.ts', (await import('./src/scrub.ts')).scrubCredentials],
  ['relay scrub.mjs', (await import('../../relay/resales-relay/scrub.mjs')).scrubCredentials],
];
...
for (const [implName, scrubCredentials] of implementations) {
  console.log(`\n══════ ${implName} ══════`);
...
console.log(failures === 0 ? '\nAll scrub tests passed (both implementations) ✓' : `\n${failures} FAILURE(S)`);
